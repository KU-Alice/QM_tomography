...
void ConvertStarlightAsciiToTree(const char* inFileName  = "starlightjpsimumudat.dat",
                        const char* outFileName = "starlight.dat")
{
  //inFileName = "starlightgammagammamumu.out";
  ...
  if (!outFile) {
	...
	while (inFile.good()) {
      ...
      outFile <<momentum[0]<< " "<<momentum[1]<<" "<<momentum[2]<<" "<< E<<" ";
