...
TFile * One_D_Corrected = TFile::Open("1D_Corrected_range.root");
//yieldTildePhi->Divide(yieldTildePhiRec,yieldTildePhiAxE);
TH1D* yieldPhi=(TH1D*) One_D_Corrected->Get("yieldPhiCorrected");
TH1D* yieldCsT=(TH1D*) One_D_Corrected->Get("yieldCstCorrected");

yieldPhi->SetLineWidth(2);
yieldPhi->SetMarkerSize(1.5);

yieldCsT->SetLineWidth(2);
yieldCsT->SetMarkerSize(1.5);

yieldCsT->GetYaxis()->SetRangeUser(0,3500);
yieldPhi->GetYaxis()->SetRangeUser(0,3500);
...
wtheta2->SetParameters(7000,1.4);
...
//tframe->Draw();
yieldCsT->Draw("e");
//yieldCsT->GetYaxis()->SetRangeUser(0,10000*factor);
...
yieldPhi->Draw("e");
//yieldPhir->Draw("e");
//yieldPhi->GetYaxis()->SetRangeUser(0,10000*factor);
...
TString m_3_2;
m_3_2.Form("m3 = %f #pm %f",pdfphi_root->GetParameter(2),pdfphi_root->GetParError(2));
...
lambda_theta.Form("#lambda_{#theta} = %f #pm %f",wphi->GetParameter(2),wphi->GetParError(2));
