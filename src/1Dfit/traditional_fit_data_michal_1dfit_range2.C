...
//#include "gROOt.h"
void traditional_fit_data_michal_1dfit_range2(int choose = 1){
...
//  YieldFile->ls();
  ...
  yieldCsTStore_read->SetTitle("Raw Distribution Costheta ");
  yieldPhiStore_read->SetTitle("Raw Distribution Phi");
  ...
  yieldCsTCorrected->Draw("texte0");
  TCanvas* canvas = new TCanvas("canvas","", 900,600);
  canvas->Divide(3,2);

  canvas->cd(1);
  yieldCsTStore_read->Draw("texte0");

  canvas->cd(2);

  yieldCsTAXE->Draw("texte0");
  ...
  canvas->cd(4);
  yieldPhiStore_read->Draw("texte0");

  canvas->cd(5);
  yieldPhiAXE->Draw("texte0");

  canvas->cd(6);
  yieldPhiCorrected->Draw("texte0");
  ...
  canvas2->cd(2);
  yieldCsTGen->Draw("texte0");
  canvas2->cd(3);
  yieldCsTRec->Draw("texte0");
  ...
  yieldPhiGen->Draw("texte0");

  canvas2->cd(6);
  yieldPhiRec->Draw("texte0");
 ...
 TFile * file = new TFile("1D_Corrected_range.root","recreate");
