...
//#include "gROOt.h"
void traditional_fit_data_michal_2dfit_step2_hel(int choose = 1){
  ...
  TH2F* yieldPhiCsRec;
  TH2F* yieldPhiCsGen;
  yieldPhiCsRec = new TH2F("yieldPhiCsRec", ";#Phi;Cos#theta",4,0,2*TMath::Pi(),3,-0.6,0.6);//(TH2F*) yieldPhiCsTStore_read->Clone("yieldPhiCsRec");
  yieldPhiCsGen = new TH2F("yieldPhiCsRec", ";#Phi;Cos#theta",4,0,2*TMath::Pi(),3,-0.6,0.6);//(TH2F*) yieldPhiCsTStore_read->Clone("yieldPhiCsGen");
  ...
  for(int i = 0;i<cm_mc_gen.daughter_neg.size();i++){
    AngleCalc * calc = new AngleCalc();
    calc->CosThetaHelicityFrame(cm_mc_gen.daughter_pos[i],cm_mc_gen.daughter_neg[i]);
    calc->PhiHelicityFrame(cm_mc_gen.daughter_pos[i],cm_mc_gen.daughter_neg[i]);
    costheta_gen.push_back(calc->ct_he);
    phi_gen.push_back(calc->phi_he);
  }

  for(int i = 0;i<cm_mc_rec.daughter_neg.size();i++){
    AngleCalc * calc = new AngleCalc();
  ...
  for (int i =0; i < phi_rec.size(); i++){

    yieldPhiCsRec->Fill(phi_rec[i],costheta_rec[i]);
  ...
  for (int i =0; i < phi_gen.size(); i++){

    yieldPhiCsGen->Fill(phi_gen[i],costheta_gen[i]);
